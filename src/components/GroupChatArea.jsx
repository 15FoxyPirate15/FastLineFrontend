...
import { Users, ArrowLeft, ArrowRight, X, Reply, Smile, CheckCheck, Check, MoreVertical, Edit2, Trash2, Copy, Hash, Phone, Video as VideoIcon, Pin, Image as ImageIcon, Paperclip, Mic, BellOff, Settings, MoreHorizontal, Link as LinkIcon, File, Headphones, Activity } from 'lucide-react';
...
const GroupChatArea = ({ groupName = "Group Chat", currentUser, onBack, socket, roomId }) => { 
  ...
  const [isGroupInfoOpen, setIsGroupInfoOpen] = useState(false); // НОВЕ: Модалка інфо групи
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (backendMessage) => {
      ...
      setMessages(prev => {
        ...
        const isMe = String(backendMessage.senderId) === String(currentUser?.id) || String(backendMessage.senderId) === String(currentUser?.email);
  ...
  return (
    <div key={roomId} className="flex flex-col h-full bg-[#05060f] text-white relative">
      ...
      <ManageMembersModal 
        ...
        roomId={roomId} groupName={groupName} participants={participants} currentUser={currentUser} refreshParticipants={fetchParticipants} onLeaveGroup={onBack}
      />

      {/* --- НОВЕ: TELEGRAM-STYLE GROUP INFO MODAL --- */}
      {isGroupInfoOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="bg-[#101426] border border-white/10 rounded-[2rem] w-full max-w-sm shadow-2xl relative z-10 overflow-hidden flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-300">
                ...
                <div className="text-center px-6 pt-4 pb-6 border-b border-white/5">
                    ...
                    {/* Кнопки дій (Telegram style) */}
                    <div className="flex items-center justify-center gap-6 mt-6">
                        <div className="flex flex-col items-center gap-2 cursor-pointer group">
                            <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center group-hover:bg-white/10 transition-colors"><BellOff size={20} className="text-gray-400 group-hover:text-white" /></div>
                        ...
                        <div className="flex flex-col items-center gap-2 cursor-pointer group" onClick={() => { setIsGroupInfoOpen(false); setIsManageModalOpen(true); }}>
                            <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center group-hover:bg-white/10 transition-colors"><Settings size={20} className="text-gray-400 group-hover:text-white" /></div>
                            <span className="text-[10px] font-bold text-gray-500 group-hover:text-gray-300 uppercase">Manage</span>
                        </div>
                        <div className="flex flex-col items-center gap-2 cursor-pointer group">
                            <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center group-hover:bg-white/10 transition-colors"><MoreHorizontal size={20} className="text-gray-400 group-hover:text-white" /></div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors group">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="text-sm font-medium text-white">fastline.app/c/295255</div>
                    ...
                    <div className="p-4 border-b border-white/5 grid grid-cols-2 gap-y-4">
                        <div className="flex items-center gap-3 cursor-pointer group"><ImageIcon size={18} className="text-gray-500 group-hover:text-[#3b82f6]" /><span className="text-sm text-gray-300 group-hover:text-white">14 Photos</span></div>
                        <div className="flex items-center gap-3 cursor-pointer group"><VideoIcon size={18} className="text-gray-500 group-hover:text-[#ec4899]" /><span className="text-sm text-gray-300 group-hover:text-white">3 Videos</span></div>
                        <div className="flex items-center gap-3 cursor-pointer group"><File size={18} className="text-gray-500 group-hover:text-[#10b981]" /><span className="text-sm text-gray-300 group-hover:text-white">12 Files</span></div>
                        <div className="flex items-center gap-3 cursor-pointer group"><Headphones size={18} className="text-gray-500 group-hover:text-[#8b5cf6]" /><span className="text-sm text-gray-300 group-hover:text-white">5 Audio</span></div>
                    </div>

                    {/* Members List (Roles & Status) */}
                    <div className="p-4">
                        <div className="text-[11px] font-black text-gray-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                            <Users size={14}/> Members
                        </div>
                        <div className="space-y-1">
                            {participants.map((p, idx) => {
                                const name = p.displayName || p.name || p.email?.split('@')[0] || "User";
                                // Імітація статусу та ролі для дизайну
                                    ...
                                    <div key={idx} className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 cursor-pointer transition-colors group/member">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-center">
                                                ...
                                                {isCreator && <span className="text-[9px] font-bold text-[#a19bfe] bg-[#6d28d9]/20 px-1.5 py-0.5 rounded border border-[#6d28d9]/30">ADMIN</span>}
                                            </div>
      ...
      <div className="h-[72px] px-6 border-b border-white/5 flex items-center justify-between bg-[#0a0f1e]/80 backdrop-blur-xl sticky top-0 z-40 shadow-sm">
        <div className="flex items-center gap-3 shrink-0">
            ...
            {/* Клік по цьому блоку відкриває Group Info */}
           ...
           if (msg.isSystem) {
             return (
               <div key={msg.id} className="flex justify-center w-full my-4 msg-enter">
                 <span className="bg-[#101426]/80 border border-white/5 px-4 py-1.5 rounded-full text-[11px] text-gray-400 font-medium shadow-sm backdrop-blur-sm">
                   {msg.text}
                 </span>
               </div>
             );
           ...
           return (
             <div key={msg.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'} group msg-enter ${isSequential ? 'mt-1' : 'mt-5'}`}>
                <div className={`flex max-w-[85%] md:max-w-[70%] gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'} relative`}>
                    ...
                    {!isMe && (
                        <div className="flex-shrink-0 mt-auto w-8 flex flex-col justify-end"> 
                            {showAvatarAndName ? (
                                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm overflow-hidden bg-gradient-to-tr from-[#1e2336] to-[#0a0b1e] border border-white/5">
                    ...
                    <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} min-w-[120px]`}>
                        {showAvatarAndName && (
                            <span className="text-[11px] font-bold text-[#a19bfe] ml-2 mb-1">{sender.name}</span>
                        )}

                        <div className={`px-4 py-2.5 text-[14px] leading-relaxed relative group/bubble transition-all ${isMe ? 'bg-gradient-to-br from-[#6d28d9] to-[#5b21b6] text-white rounded-2xl rounded-br-sm shadow-md' : 'bg-[#161b33] text-gray-100 rounded-2xl rounded-bl-sm border border-white/5 shadow-sm'}`}>
                            
                            {msg.replyTo && (
                                <div className={`mb-2 pl-3 border-l-[3px] text-[11px] rounded-r-lg py-1 cursor-pointer transition-colors ${isMe ? 'border-white/50 bg-black/10 text-white' : 'border-[#a19bfe] bg-black/20 text-gray-300'}`}>
      ...
      <div className="bg-[#0a0f1e]/90 backdrop-blur-2xl border-t border-white/5 p-4 z-20 relative shrink-0 w-full">
        ...
        <div className="max-w-4xl mx-auto w-full">
            <div className="bg-[#101426] rounded-2xl flex items-center px-2 py-1.5 border border-white/5 shadow-inner focus-within:border-[#6d28d9]/50 focus-within:ring-2 focus-within:ring-[#6d28d9]/20 transition-all">
                <button className="p-2.5 text-gray-500 hover:text-[#a19bfe] transition-colors rounded-xl hover:bg-white/5">
                    <ImageIcon size={20} />
                </button>

                <input 
                    className="flex-1 bg-transparent outline-none text-white text-sm px-2 py-2.5 h-11 placeholder-gray-500 min-w-0" 
                ...
                <div className="flex items-center gap-1 pr-1">
                    ...
                    {input.trim() || editingMessage ? (
                        <button onClick={handleSend} className="p-2.5 bg-gradient-to-r from-[#6d28d9] to-[#3b82f6] text-white rounded-xl shadow-[0_0_15px_rgba(109,40,217,0.4)] active:scale-95 transition-all animate-in zoom-in duration-200">
                            {editingMessage ? <Check size={18} /> : <ArrowRight size={18} />}
                        </button>
                    ) : (
                        <button className="p-2.5 text-gray-500 hover:text-[#a19bfe] transition-colors rounded-xl hover:bg-white/5 animate-in zoom-in duration-200">
                            <Mic size={20} />
                        </button>
                    )}
                </div>
